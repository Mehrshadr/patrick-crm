    ...
    public function find_media_by_url($request)
    {
        ...
        if ($table_exists) {
            ...
            if (!empty($parsed['path'])) {
                ...
                if ($attachment_id) {
                    $this->log("[find_media] Found via filename: $attachment_id");
                    return rest_ensure_response([
                        'success' => true,
                        'media_id' => intval($attachment_id)
                    ]);
                ...
                $sample = $wpdb->get_row("SELECT source_id, path FROM $as3cf_table LIMIT 1");
                if ($sample) {
                    $this->log("[find_media] Sample row - source_id: {$sample->source_id}, path: {$sample->path}");
                } else {
